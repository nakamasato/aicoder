...
func Command() *cobra.Command {
	planCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			prompt, err := generatePrompt(ctx, entClient, goal, config.Repository)
			if err != nil {
...
func generatePrompt(ctx context.Context, entClient *ent.Client, goal, repo string) (string, error) {
...
// goalFromPrompt extracts the goal from the prompt.
func goalFromPrompt(prompt string) string {
	// ここでは単純に3行目以降をGoalとして抽出します。
	lines := strings.Split(prompt, "\n")
	for _, line := range lines {
		if strings.HasPrefix(line, "My goal is:") {
			return strings.TrimSpace(strings.TrimPrefix(line, "My goal is:"))
		}
	}
	return ""
}

// parsePlanText parses plain text plan into Steps.
func parsePlanText(planText string) []Step {
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(line, "- ") {
			...
			parts := strings.SplitN(stepContent, ": ", 2)
			if len(parts) == 2 {
				step := Step{
					Name: parts[0],
					Tool: parts[1],
...
// savePlanToFile saves the generated plan to a JSON file.
func savePlanToFile(plan PlanResult, filename string) error {
